...
class ModelValidator:
    ...
    def validate_feedback_model(validation_dict, subject_ids, training_type, algorithm, t, score_dict, leave_one_out_id, model_creation_dict):
        ...
        for humanboneindex_name, validation_df in validation_dict.items():
            model = load(thisdir + "/core/ml_models/" + training_type + "/" + algorithm + "/no_magn/" + subject_ids + "_" + humanboneindex_name + "_" + t)
    ...
    def plot_score_dict(score_dict, training_type, algorithm):
        ...
        plt.legend()
        plt.savefig(thisdir + "/core/analysis/scores/" + training_type + "_" + algorithm + "_scores_no_magn" + "_" + str(int(time.time())) + ".png")     
        plt.clf()
        f = open(thisdir + "/core/analysis/scores/" + training_type + "_" + algorithm + "_scores" + "_values_no_magn_" + str(int(time.time())) + ".txt", "w")
