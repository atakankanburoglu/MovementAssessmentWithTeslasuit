    ...
    def plot_feedback_result_heatmaps(self, result):
        ...
        for name, group in gb_df_to_plot: 
            ...
            if humanboneIndex_name != name:
                ...
                plt.savefig(thisdir + "/core/analysis/relative_errors/" + exercise_type + "/" + algorithm + "/" + subject_ids + "_" + humanboneindex_name + "_" + t + "_relative_errors_heatmap.png", dpi=1200)     
                plt.clf()
                humanboneIndex_name = name
                df = group
            else:
        ...
        plt.figure(figsize=(10, 10))
        sns.heatmap(df_matrix)
        plt.savefig(thisdir + "/core/analysis/relative_errors/" + exercise_type + "/" + algorithm + "/" + subject_ids + "_" + humanboneindex_name + "_" + t + "_relative_errors_heatmap.png")     
        plt.clf()
        print("Finished saving heatmaps")

    @staticmethod
    def plot_feedback_result_heatmaps(self, relative_errors, model_data):
        thisdir = os.getcwd()
        t = time.time()
        relative_errors['Timestamp'] = relative_errors['Timestamp'].min()

        gb_relative_errors = relative_errors.groupby(['HumanBoneIndex_Axis'])
        humanboneIndex_name = ""
        df = pd.DataFrame()
        for name, group in gb_relative_errors: 
            name = name[0].split('_')[0]
            if humanboneIndex_name == "":
                humanboneIndex_name = name
            if humanboneIndex_name != name:
                ...
                plt.figure(figsize=(10, 10))
                sns.heatmap(df_matrix)
                plt.savefig(thisdir + "/core/analysis/relative_errors/" + model_data.exercise_type + "/" + model_data.algorithm + "/" + model_data.subject_ids + "_" + humanboneindex_name + "_" + model_data.timestamp + "_relative_errors_heatmap.png", dpi=1200)     
                plt.clf()
                humanboneIndex_name = name
                df = group
            else:
        ...
        plt.figure(figsize=(10, 10))
        sns.heatmap(df_matrix)
        plt.savefig(thisdir + "/core/analysis/relative_errors/" + model_data.exercise_type + "/" + model_data.algorithm + "/" + model_data.subject_ids + "_" + humanboneindex_name + "_" + model_data.timestamp + "_relative_errors_heatmap.png")     
        plt.clf()
        print("Finished saving heatmaps")
    ...
    def plot_feedback_result_barcharts(self, df_to_plot, filename, rows):
        ...
        for name, group in gb_df_to_plot: 
            ...
            if humanboneIndex_name == "":
                humanboneIndex_name = name
            if humanboneIndex_name != name:
                df['label'] = pd.cut(df['Error'],bins=[-100, -50,  -10, -1, 1, 10, 50, 100], labels=['-100 < Error < -50', '-50 < Error < -10', '-10 < Error < -1', '-1 < Error < 1', '1 < Error < 10' , '10 < Error < 50' '10 < Error < 100'])
