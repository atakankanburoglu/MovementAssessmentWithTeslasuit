...
class ModelEvaluator:
    ...
    @staticmethod
    def error_to_direction(model_data, relative_errors):
        vectorBins = {
            "up" : np.array([0, 1, 0]),
            "down" : np.array([0, -1, 0]),
        ...
        columns = ["HBI"]
        columns.extend(needed_axes_lst)  
        df = pd.DataFrame(relative_errors, columns=columns)
        for m_set in ["magn"]: #m_sets:
            data = ""
            for hbi, hbi_df in df.groupby(["HBI"]):
                m_set_df = hbi_df.loc[:, hbi_df.columns.str.contains(m_set)]
                entries = tuple(m_set_df.iloc[0].values)
                if (len(entries) > 3  and (list(entries)[1:] != [0, 0, 0] and list(entries) != [0, 0, 0, 0])) or (len(entries) <= 3 and list(entries) != [0, 0, 0]):   
                    if len(entries) > 3:
                        quat = Quaternion(axis=list(entries)[1:], angle=entries[0])
                    ...
                    for bin in vectorBins:
                        ...
                        if angle < min_angle or min_bin == "":
                            min_bin = bin
                            min_angle = angle
                    data = data + "(" + hbi[0] + "," + str(vectorBins[min_bin][0]) + "," + str(vectorBins[min_bin][1]) + "," + str(vectorBins[min_bin][2]) + "," + str(np.round(angle, 2)) + "),"
        return data
